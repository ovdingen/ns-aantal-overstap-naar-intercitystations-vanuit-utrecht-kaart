...
$insertquery = $db->prepare("INSERT INTO nsstations VALUES(:stationcode, :stationname, :station_type, :station_lat, :station_lon)"); // Prepare the SQL statement. A prepared statement can be reused infinitely, so we're going to make use of this so we don't need to create a prepared statement for every train station.
...
foreach($xml as $k => $v) {
    ...
    $data['station_lat'] = (float) $v->Lat;
    $data['station_lon'] = (float) $v->Lon;
